...
function listar($db){
	...
	//echo $filtro;
	$sql = $db->query("SELECT a.*, e.descripcion as nomEspecialidad from alumnos a inner join especialidad e on e.id = a.idEspecialidad where a.activo =1 {$filtro} order by apellidos asc;");
	if($sql->execute()){
